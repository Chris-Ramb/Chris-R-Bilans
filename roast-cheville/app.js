(() => {
  ...
  const forceDefs = [
    ...
    { key: "hipAbd", label: "Abducteur de hanche" },
  ...
  // ✅ Correctif navigation : force tous les boutons en type="button"
  ...
  function bindWizard() {
    ...
    $$("[data-step-jump]").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        goToStep(Number(btn.dataset.stepJump));
      });
    });
  }
  ...
  function liveRecalc(target) {
    ...
    if (id.startsWith("ott")) updateOttawa();
    if (id.startsWith("syn")) updateSyndesmose();
    ...
    if (id === "sls_r" || id === "sls_l") updateSingleLegStance();
    if (id.startsWith("ybt_")) updateYBT();
    ...
    if (["patientCode","patientAge","sideLesion","traumaDate","evalDate"].includes(id)) {
      updateQfaamQr();
    }
  ...
  function updateSingleLegStance() {
    const r = parseNum($("sls_r")?.value);
    const l = parseNum($("sls_l")?.value);
    if (r === null || l === null) {
      $("sls_delta").value = "";
      return;
    }
    ...
    if (unilateral) {
      $("sls_delta").value = fmtNum(lesionVal - healthyVal, 0);
    } else {
      $("sls_delta").value = fmtNum(r - l, 0);
    }

    if ($("slsDeltaLabel")) {
      $("slsDeltaLabel").textContent = (lesion === "Droite" || lesion === "Gauche")
        ? "Delta (lésé - sain)"
        : "Delta (Droite - Gauche)";
    }
  }

  function getYbtMean(side, dir) {
    return meanOf([
      $(`ybt_${side}_${dir}_1`)?.value,
      $(`ybt_${side}_${dir}_2`)?.value,
      $(`ybt_${side}_${dir}_3`)?.value
    ]);
  }

  function updateYBT() {
    const lenR = parseNum($("ybt_len_r")?.value);
    const lenL = parseNum($("ybt_len_l")?.value);

    const dirs = ["ant", "pm", "pl"];

    const means = {
      r: {
        ant: getYbtMean("r", "ant"),
        pm: getYbtMean("r", "pm"),
        pl: getYbtMean("r", "pl")
      },
      l: {
        ant: getYbtMean("l", "ant"),
        pm: getYbtMean("l", "pm"),
        pl: getYbtMean("l", "pl")
      }
    };
    ...
    dirs.forEach((dir) => {
      const mR = means.r[dir];
      const mL = means.l[dir];

      $("ybt_norm_r_" + dir).value = (mR !== null && lenR && lenR > 0) ? fmtNum((mR / lenR) * 100, 1) : "";
      $("ybt_norm_l_" + dir).value = (mL !== null && lenL && lenL > 0) ? fmtNum((mL / lenL) * 100, 1) : "";
    });

    const compRReady = lenR && lenR > 0 && dirs.every((d) => means.r[d] !== null);
    const compLReady = lenL && lenL > 0 && dirs.every((d) => means.l[d] !== null);

    $("ybt_comp_r").value = compRReady
      ? fmtNum(((means.r.ant + means.r.pm + means.r.pl) / (3 * lenR)) * 100, 1)
      : "";

    $("ybt_comp_l").value = compLReady
      ? fmtNum(((means.l.ant + means.l.pm + means.l.pl) / (3 * lenL)) * 100, 1)
      : "";
  }
  ...
  function forceTableHTML(def) {
    return `
      <div class="card" data-force-block="${def.key}">
        <h3 class="block-title">${def.label}</h3>
          ...
          <table data-force-table="${def.key}" data-force-label="${def.label}">
            <thead>
              <tr>
                ...
                <th>NR</th>
                <th>Motif NR</th>
                <th><span data-side-label="right"></span> E1</th>
                <th><span data-side-label="right"></span> E2</th>
                <th><span data-side-label="right"></span> E3</th>
                <th>Douleur <span data-side-short="right"></span></th>
                <th><span data-side-label="left"></span> E1</th>
                <th><span data-side-label="left"></span> E2</th>
                <th><span data-side-label="left"></span> E3</th>
                <th>Douleur <span data-side-short="left"></span></th>
                <th>Retenu <span data-side-short="right"></span></th>
                <th>Retenu <span data-side-short="left"></span></th>
  ...
  function tricepsBlockHTML() {
    return `
      <div class="card" data-force-block="triceps">
        <h3 class="block-title">Triceps sural</h3>

        <div style="font-weight:600; margin:4px 0 8px;">1) Single calf raise</div>
        <div class="grid g4">
          <div class="field">
            <label><span data-side-label="right"></span> (nb)</label>
            <input id="calfStdRight" type="number" min="0" step="1">
          </div>
          <div class="field">
            <label><span data-side-label="left"></span> (nb)</label>
  ...
  function initForceSection() {
    ...
    const intro = `
      <div class="card">
        <h3 class="block-title">Paramètres de force</h3>
  ...
  function updateSideLabelsUI() {
    const labels = getSideLabels();

    $$('[data-side-label="right"]').forEach(el => { el.textContent = labels.right; });
    $$('[data-side-label="left"]').forEach(el => { el.textContent = labels.left; });

    $$('[data-side-short="right"]').forEach(el => {
      el.textContent = labels.right.includes("(lésé)") ? "D*" : "D";
    });
    $$('[data-side-short="left"]').forEach(el => {
    ...
    if ($("slsDeltaLabel")) $("slsDeltaLabel").textContent = deltaHead;
  ...
  function refreshAutoSummary() {
    ...
    const slsDelta = parseNum($("sls_delta")?.value);
    if (slsDelta !== null) {
      messages.push(`- Contrôle postural statique (Single Leg Stance 20 s): delta erreurs ${fmtNum(slsDelta,0)}.`);
    }

    const ybtCompR = safeText($("ybt_comp_r")?.value);
    const ybtCompL = safeText($("ybt_comp_l")?.value);
    if (ybtCompR || ybtCompL) {
      messages.push(`- Contrôle postural dynamique (YBT): composite normalisé ${ybtCompR ? `Droite ${ybtCompR}%` : ""}${ybtCompR && ybtCompL ? " ; " : ""}${ybtCompL ? `Gauche ${ybtCompL}%` : ""}.`);
    }
    ...
    return {
      ...
      postural: {
        sls: {
          right: $("sls_r")?.value || "",
          left: $("sls_l")?.value || "",
          delta: $("sls_delta")?.value || "",
          comment: $("sls_comment")?.value || ""
        },
        ybt: {
          lenR: $("ybt_len_r")?.value || "",
          lenL: $("ybt_len_l")?.value || "",
          normR_ant: $("ybt_norm_r_ant")?.value || "",
          normR_pm: $("ybt_norm_r_pm")?.value || "",
          normR_pl: $("ybt_norm_r_pl")?.value || "",
          compR: $("ybt_comp_r")?.value || "",
          normL_ant: $("ybt_norm_l_ant")?.value || "",
          normL_pm: $("ybt_norm_l_pm")?.value || "",
          normL_pl: $("ybt_norm_l_pl")?.value || "",
          compL: $("ybt_comp_l")?.value || "",
          comment: $("ybt_comment")?.value || ""
        }
      },
  ...
  function buildPrintSummary() {
    ...
    const html = `
      ...
      <div class="ps-card">
        <h2>Contrôle postural</h2>
        <table><tbody>
          ${rowKV("Single Leg Stance 20 s", [
            `${rightHead}: ${d.postural.sls.right} erreurs`,
            `${leftHead}: ${d.postural.sls.left} erreurs`,
            d.postural.sls.delta && `Delta: ${d.postural.sls.delta}`,
            d.postural.sls.comment
          ].filter(Boolean).join(" | "))}
          ${rowKV("YBT longueur MI (cm)", [
            `${rightHead}: ${d.postural.ybt.lenR}`,
            `${leftHead}: ${d.postural.ybt.lenL}`
          ].filter(Boolean).join(" | "))}
          ${rowKV(`YBT ${rightHead} (%LL)`, [
            d.postural.ybt.normR_ant && `ANT ${d.postural.ybt.normR_ant}%`,
            d.postural.ybt.normR_pm && `PM ${d.postural.ybt.normR_pm}%`,
            d.postural.ybt.normR_pl && `PL ${d.postural.ybt.normR_pl}%`,
            d.postural.ybt.compR && `Composite ${d.postural.ybt.compR}%`
          ].filter(Boolean).join(" | "))}
          ${rowKV(`YBT ${leftHead} (%LL)`, [
            d.postural.ybt.normL_ant && `ANT ${d.postural.ybt.normL_ant}%`,
            d.postural.ybt.normL_pm && `PM ${d.postural.ybt.normL_pm}%`,
            d.postural.ybt.normL_pl && `PL ${d.postural.ybt.normL_pl}%`,
            d.postural.ybt.compL && `Composite ${d.postural.ybt.compL}%`
          ].filter(Boolean).join(" | "))}
          ${rowKV("Commentaire YBT", d.postural.ybt.comment)}
        </tbody></table>
      </div>
  ...
  function resetAll(askConfirm = true) {
    ...
    [
      "ottConclusionAuto","synConclusionAuto",
      "edemaDelta","edemaFlag",
      "lungeCmDelta","lungeCmFlag","lungeDegDelta",
      "jxDisplay",
      "sls_delta",
      "ybt_norm_r_ant","ybt_norm_r_pm","ybt_norm_r_pl","ybt_comp_r",
      "ybt_norm_l_ant","ybt_norm_l_pm","ybt_norm_l_pl","ybt_comp_l"
    ].forEach(id => {
      if ($(id)) $(id).value = "";
    });
  ...
  function boot() {
    normalizeAllButtons(); // ✅ très important pour la navigation
    bindWizard();
    bindDirtyTracking();
    bindTopButtons();
